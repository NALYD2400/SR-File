        ...
        private void ReportForm_Load(object sender, EventArgs e)
        {
            ...
            for (int i = appLog.Entries.Count - 1; i >= 0; i--)
            {
                ...
                if (entry.EntryType == EventLogEntryType.Error && entry.Source == ".NET Runtime")
                {
                    ...
                    if (lines.Length > 0 &&
                        (lines[0].Contains("SR File.exe") ||
                         lines[0].Contains("SR File Explorer.exe") ||
                         lines[0].Contains("SR File Ped Viewer.exe") ||
                         lines[0].Contains("SR File Vehicle Viewer.exe")))
                    {
                        found = true;
            ...
            if (!found)
            {
                ...
                MessageBox.Show("Unable to find the last SR File error in the Event Log.");
            }
        }
