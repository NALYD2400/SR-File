...
namespace CodeWalker
{
    ...
    public class InputManager
    {
        ...
        public void KeyDown(KeyEventArgs e, bool enablemove)
        {
            ...
            //WASD / ZQSD / Flèches move (AZERTY + QWERTY support natif)...
            if (enablemove)
            {
                if (k == keyBindings.MoveForward || k == Keys.Up || k == Keys.W || k == Keys.Z) kbmovefwd = true;
                if (k == keyBindings.MoveBackward || k == Keys.Down || k == Keys.S) kbmovebck = true;
                if (k == keyBindings.MoveLeft || k == Keys.Left || k == Keys.A || k == Keys.Q) kbmovelft = true;
                if (k == keyBindings.MoveRight || k == Keys.Right || k == Keys.D) kbmovergt = true;
        ...
        public void KeyUp(KeyEventArgs e)
        {
            ...
            var k = e.KeyCode;
            if (k == keyBindings.MoveForward || k == Keys.Up || k == Keys.W || k == Keys.Z) kbmovefwd = false;
            if (k == keyBindings.MoveBackward || k == Keys.Down || k == Keys.S) kbmovebck = false;
            if (k == keyBindings.MoveLeft || k == Keys.Left || k == Keys.A || k == Keys.Q) kbmovelft = false;
            if (k == keyBindings.MoveRight || k == Keys.Right || k == Keys.D) kbmovergt = false;
    ...
    public class KeyBindings
    {
        ...
        public bool IsAzertyMode => MoveForward == Keys.Z && MoveLeft == Keys.Q;
        ...
        public static bool IsFrenchKeyboard()
        {
            try
            {
                ...
                if (lang != null)
                {
                    if (lang.Culture != null && lang.Culture.TwoLetterISOLanguageName.Equals("fr", StringComparison.OrdinalIgnoreCase))
                        return true;
                    if (!string.IsNullOrEmpty(lang.LayoutName) && lang.LayoutName.IndexOf("AZERTY", StringComparison.OrdinalIgnoreCase) >= 0)
                        return true;
                }
            }
            catch { }
        ...
        public void ApplyAzerty()
        {
            ...
            MoveUp = Keys.R;
            MoveDown = Keys.F;
            MoveSlowerZoomIn = Keys.W;
            MoveFasterZoomOut = Keys.X;
            ToggleMouseSelect = Keys.C;
            ToggleToolbar = Keys.T;
            ExitEditMode = Keys.A;
            EditPosition = Keys.Z;
            EditRotation = Keys.E;
            EditScale = Keys.R;
            ...
            if (sc != null && sc.Count > 0)
            {
                foreach (string s in sc)
                {
                    ...
                    if (parts.Length == 2)
                    {
                        ...
                        Keys k;
                        if (Enum.TryParse<Keys>(sval, out k))
                        {
                ...
                // If user has a French keyboard and the loaded bindings were the old default QWERTY (W and A),
                // automatically convert them to AZERTY (Z and Q) so French users don't need to change Windows layout!
                if (IsFrenchKeyboard() && MoveForward == Keys.W && MoveLeft == Keys.A)
                {
                    ApplyAzerty();
                }
            }
