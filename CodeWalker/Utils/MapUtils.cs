...
namespace CodeWalker
{
    ...
    public class MapIcon
    {
        ...
        public MapIcon(string name, string filepath, int texw, int texh, float centerx, float centery, float scale)
        {
            ...
            {
                string alt = PathUtil.GetFilePath(Path.GetFileName(filepath));
                if (File.Exists(alt)) filepath = alt;
                else
                {
                    string iconsAlt = PathUtil.GetFilePath("icons\\" + Path.GetFileName(filepath));
                    if (File.Exists(iconsAlt)) filepath = iconsAlt;
                }
            }
            ...
            if (!File.Exists(filepath))
            {
                try
                {
                    ...
                    using (var bmp = new System.Drawing.Bitmap(texw > 0 ? texw : 32, texh > 0 ? texh : 32))
                    {
                        using (var g = System.Drawing.Graphics.FromImage(bmp))
                        {
                            g.Clear(System.Drawing.Color.FromArgb(229, 26, 60));
                        }
