...
namespace CodeWalker
{
    public static class GTAFolder
    {
        ...
        static GTAFolder()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(CurrentGTAFolder) || !ValidateGTAFolder(CurrentGTAFolder, IsGen9))
                {
                    string auto = AutoDetectFolder();
                    if (!string.IsNullOrWhiteSpace(auto) && ValidateGTAFolder(auto, IsGen9Folder(auto)))
                    {
                        CurrentGTAFolder = auto;
                        IsGen9 = IsGen9Folder(auto);
                        Settings.Default.GTAFolder = auto;
                        Settings.Default.GTAGen9 = IsGen9;
        ...
        public static bool UpdateGTAFolder(bool useCurrentIfValid = false, bool autoDetect = true)
        {
            ...
            if (autoDetect)
            {
                ...
                if (autoFolder != null && MessageBox.Show($"Dossier de jeu détecté automatiquement :\n\"{autoFolder}\" ({source}).\n\nUtiliser ce dossier GTA V ?", "GTA V Détecté", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
                {
                    bool gen9detected = IsGen9Folder(autoFolder);
                    if (ValidateGTAFolder(autoFolder, gen9detected))
                    {
                        SetGTAFolder(autoFolder, gen9detected);
                        return true;
                    }
                    f.SelectedFolder = autoFolder;
                    f.IsGen9 = gen9detected;
                }
            }
        ...
        public static bool AutoDetectFolder(out Dictionary<string, string> matches)
        {
            ...
            {
                matches.Add("Current SR File Folder", CurrentGTAFolder);
            }
