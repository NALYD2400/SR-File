...
namespace CodeWalker.World
{
    public partial class SettingsForm : Form
    {
        ...
        public SettingsForm(WorldForm owner)
        {
            ...
            var azertyPresetBtn = new Button
            {
                Text = "🇫🇷 AZERTY (ZQSD)",
            ...
            var qwertyPresetBtn = new Button
            {
                Text = "🇬🇧 QWERTY (WASD)",
            ...
            azertyPresetBtn.Click += (s, e) =>
            {
                keyBindings.ApplyAzerty();
                LoadKeyBindings();
                Settings.Default.KeyBindings = keyBindings.GetSetting();
                Settings.Default.Save();
                if (worldForm != null) worldForm.SetKeyBindings(keyBindings);
                MessageBox.Show(this, "Disposition française AZERTY (ZQSD) activée !", "SR File", MessageBoxButtons.OK, MessageBoxIcon.Information);
            };

            qwertyPresetBtn.Click += (s, e) =>
            {
                keyBindings.ApplyQwerty();
                LoadKeyBindings();
                Settings.Default.KeyBindings = keyBindings.GetSetting();
                Settings.Default.Save();
                if (worldForm != null) worldForm.SetKeyBindings(keyBindings);
                MessageBox.Show(this, "Disposition anglaise QWERTY (WASD) activée !", "SR File", MessageBoxButtons.OK, MessageBoxIcon.Information);
            };

            groupBox1.Controls.Add(azertyPresetBtn);
            groupBox1.Controls.Add(qwertyPresetBtn);
        ...
        {
            if (MessageBox.Show("Are you sure you want to reset all SR File settings to their default values?", "Confirm reset settings to defaults", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
